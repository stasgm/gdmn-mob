...
import { IInvoiceDocument } from '../../store/types';
...
interface IFilteredList {
  searchQuery: string;
  list: IInvoiceDocument[];
}

export interface InvoiceListSectionProps {
  title: string;
}
export type SectionDataProps = SectionListData<IListItemProps, InvoiceListSectionProps>[];

export const InvoiceListScreen = () => {
  const navigation = useNavigation<StackNavigationProp<InvoiceStackParamList, 'InvoiceList'>>();
  ...
  const list = useSelector((state) => state.documents.list) as IInvoiceDocument[];
    ...
    React.useCallback(() => {
      if (!searchQuery) {
        setFilteredList({
          searchQuery,
          list: list
            .filter((i) => i.documentType.name === 'invoice')
  ...
  useEffect(() => {
    if (searchQuery !== filteredList.searchQuery) {
      if (!searchQuery) {
        ...
        const fn = ({ head, documentDate, number }: IInvoiceDocument) =>
          head.depart?.name?.toLowerCase().includes(lower) ||
