...
import { IWaybillDocument } from '../../store/types';
...
interface IFilteredList {
  searchQuery: string;
  list: IWaybillDocument[];
}

export interface WaybillListSectionProps {
  title: string;
}
export type SectionDataProps = SectionListData<IListItemProps, WaybillListSectionProps>[];

export const WaybillListScreen = () => {
  const navigation = useNavigation<StackNavigationProp<WaybillStackParamList, 'WaybillList'>>();
  ...
  const list = useSelector((state) => state.documents.list) as IWaybillDocument[];
  ...
  const handleAddDocument = useCallback(() => {
    navigation.navigate('WaybillEdit');
  }, [navigation]);
    ...
    React.useCallback(() => {
      if (!searchQuery) {
        setFilteredList({
          searchQuery,
          list: list
            .filter((i) => i.documentType.name === 'waybill')
  ...
  useEffect(() => {
    if (searchQuery !== filteredList.searchQuery) {
      if (!searchQuery) {
        ...
        const fn = ({ head, documentDate, number }: IWaybillDocument) =>
  ...
  const newFilteredList: IListItemProps[] = useMemo(() => {
    ...
    return res.map((i) => {
      return {
        ...
        // subtitle: i.documentType.description || '',
      } as IListItemProps;
    });
      ...
      <ScreenListItem
        ...
        onPress={() =>
          isDelList
            ...
            : navigation.navigate('WaybillView', { id: item.id })
        }
        onLongPress={() => setDelList(getDelList(delList, item.id, item.status!))}
        checked={!!delList[item.id]}
        addInfo={
          <View>
            {doc.head.depart && <MediumText>{doc.head.depart.name}</MediumText>}
            <MediumText>
