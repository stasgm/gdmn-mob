...
export type SectionDataProps = SectionListData<IListItemProps, MoveFromListSectionProps>[];

export const MoveFromListScreen = () => {
  const navigation = useNavigation<StackNavigationProp<MoveFromStackParamList, 'MoveFromList'>>();
  const docDispatch = useDocThunkDispatch();

  const list = (
    useSelector((state) => state.documents.list)?.filter(
      (i) => i.documentType?.name === 'movement' && i.head?.subtype.id === 'movement',
    ) as IMoveDocument[]
      ...
      <ScreenListItem
        ...
        onPress={() =>
          isDelList
            ...
            : navigation.navigate('MoveFromView', { id: item.id })
        }
