...
export const LaboratoryEditScreen = () => {
  ...
  const shipments = useFilteredDocList<ILaboratoryDocument>('laboratory');
  ...
  useEffect(() => {
    if (screenState === 'saving') {
      ...
      if (!id) {
        ...
        navigation.dispatch(StackActions.replace('LaboratoryView', { id: newDoc.id }));
      } else {
        if (!doc) {
          setScreenState('idle');
          return;
        ...
        const updatedDoc: ILaboratoryDocument = {
          ...doc,
          id,
          number: docNumber && docNumber.trim(),
        ...
        navigation.navigate('LaboratoryView', { id });
      }
      setScreenState('idle');
    }
