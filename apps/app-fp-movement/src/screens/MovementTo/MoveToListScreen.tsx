...
export interface MoveToListSectionProps {
  title: string;
}

export type SectionDataProps = SectionListData<IListItemProps, MoveToListSectionProps>[];
...
export const MoveToListScreen = () => {
  ...
  const list = (
    useSelector((state) => state.documents.list)?.filter(
      (i) => i.documentType?.name === 'movement' && i.head?.subtype.id === 'internalMovement',
    ) as IMoveDocument[]
  ...
  const handleAddDocument = useCallback(() => {
    navigation.navigate('MoveToEdit');
  }, [navigation]);
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      ...
      title: isDelList ? `Выделено документов: ${Object.values(delList).length}` : 'На хранение',
    });
      ...
      <ScreenListItem
        ...
        onPress={() =>
          isDelList
            ...
            : navigation.navigate('MoveToView', { id: item.id })
        }
