...
import { palletScreens } from './Root/screens';

const Stack = createStackNavigator<PalletStackParamList>();

export const PalletNavigator = () => {
  return (
    <Stack.Navigator initialRouteName="PalletList" screenOptions={{ headerShown: true, headerBackTitleVisible: false }}>
      {Object.entries({ ...palletScreens }).map(([name, { title, component }]) => (
