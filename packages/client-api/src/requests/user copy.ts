...
//   addUser = async (
//     user: NewUser,
//     customRequest: RobustRequest,
//   ): Promise<error.INetworkError | types.IAddUserResponse> => {
...
//     const res = await customRequest({ method: 'POST', url: '/users', data: user });
...
//       type: 'ERROR',
//       message:
//         res.result === 'ERROR' || res.result === 'INVALID_DATA'
//           ? res.message || 'ошибка добавления пользователя'
...
//   getUser = async (userId: string, customRequest?: RobustRequest) => {
//     if (this.api.config.debug?.isMock) {
//       await sleep(this.api.config.debug?.mockDelay || 0);
...
//     const res = await robustRequest({ method: 'GET', url: `/users${userId}` });
...
//   getUsers = async (customRequest: RobustRequest, params?: Record<string, string | number>) => {
//     if (this.api.config.debug?.isMock) {
//       await sleep(this.api.config.debug?.mockDelay || 0);
...
//     const res = await customRequest({ method: 'POST', url: '/users', params });

//     if (res.result === 'OK') {
//       return {
//         type: 'GET_USERS',
//         users: res.response.data?.data,
//       } as types.IGetUsersResponse;
//     }
...
//       message:
//         res.result === 'ERROR' || res.result === 'INVALID_DATA'
//           ? res.message || 'ошибка получения данных о пользователях'
