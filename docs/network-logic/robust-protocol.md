...
3. Если записей нет, то определяем список сообщений для передачи на Гедымин. Если он пуст, то передаем статус `IDLE`, получив который Гедымин завершит у себя выполенение макроса. Если присутствует хотя бы одно сообщение для откправки, то процесс можно начинать:
