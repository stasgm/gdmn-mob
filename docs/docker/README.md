...
2. В корне проекта в .env файле указать значения google ключей для капчи, которые будут переданы при сборке в переменные среды через аргументы
...
Важно, порт HTTPS_PORT или HTTP_PORT при запуске образа сервера должен совпадать с портом, указанным в библиотеке client-config при формировании образа админки.
...
Если надо сбилдить только образ админки для клиента, то необходимо:
1. Клонируем репозиторий в папку workspace (предварительно выполнить команду sudo -s и после дать права на папку chown -R $USER gdmn-mob):
```
sudo -s
git clone https://github.com/stasgm/gdmn-mob.git
