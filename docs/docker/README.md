...
scp -P 'port'-r 'локальная_папка' 'пользователь@удаленный_сервер:путь_на_удаленном_сервере': Копирует файлы и папки с локальной машины на удаленный сервер по SSH.
