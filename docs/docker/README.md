...
5. Устанавливаем версии сервисов в файле docker-compose.build.yaml, например, gsbelarus/gdmn-mob.web-admin:1.1.1, gsbelarus/gdmn-mob.server:1.0.0

6. Из папки gdmn-mob выполняем команду создания образов:
```
...
Чтобы Docker Compose загрузил образы из Docker Hub и запустил соответствующие контейнеры, выполните следующую команду из директории, содержащей файл docker-compose.yml:
```
docker-compose up
```
Если запуск в фоновом режиме
```
docker-compose up -d
```
