...
5. Если уже существует активный процесс для переданного ИД базы данных, то возвращается ответ со статусом `BUSY`. Получив данный статус:
    ...
    5. Таким образом мы избегаем ситуации, когда два и более процесса Гедымина, подключенных к одной и той же базе данных, будут одновременно получать из очереди сообщения и выполнять действия на базе данных. Этим мы минимизируем риск возникновения dead lock при изменении одних и тех же записей.
