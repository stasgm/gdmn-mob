...
Схематически, взаимодействие системы **GDMN. Отгрузка** с модулями информационной системы предприятия выглядит следующим образом:

```mermaid

flowchart TD;
    A(Производство)--Приемка ГП-->B[Склад ГП];
    B--Перемещение на склад хранения-->C(Ячейки, склад адресного хранения);
    B--Отгрузка покупателю-->E;
    C--Перемещение со склада хранения-->B;
    B--Наличие ГП-->M;
    B--Отгрузка по заявке-->D[Экспедиция];
    D--Доставка-->E(Покупатель);
    E--Учет оплаты-->Acc(Бухгалтерия);
    M--Задание на отгрузку ГП-->B;
    Acc--Сальдо клиента-->E;
    P(ПЭО)--Цены-->M;
    M--Производственное задание-->A;
    B--Учет отгрузки-->Acc;
    M(Отдел маркетинга)--Аналитические отчеты-->Mngmt(Руководство);
...
<img src="img/2.Settings/2.Synchr.jpg" alt="drawing" height="400"/>
...
<img src="img/3.Docs/3.AddDocTypes_AS.jpg" alt="drawing" height="400"/> <img src="img/3.Docs/3.MovementAdd.jpg" alt="drawing" height="400"/> <img src="img/3.Docs/3.InternalMovementAdd.jpg" alt="drawing" height="400"/>
